...
RSpec.describe 'public/submissions/new', type: :view do
  before(:each) do
  ...
  it 'renders new public_submission form' do
    render

    assert_select 'h1', 'MyString'
    assert_select 'form[action=?][method=?]', submissions_path(locale: @locale, layer_id: @layer.id), 'post' do
