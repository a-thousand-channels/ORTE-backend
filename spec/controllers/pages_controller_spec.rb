  ...
  describe "functionalities with logged in user with role 'admin'" do
    ...
    describe 'GET #index' do
      it 'returns a success response' do
        page = Layer.create! valid_attributes
    ...
    describe 'GET #import' do
      it 'renders the import form' do
        get :import, params: { map_id: @map.id, id: page.friendly_id }, session: valid_session
    ...
    describe 'POST #import_preview' do
      ...
      context 'with valid CSV' do
        it 'renders the import preview' do
          post :import_preview, params: { map_id: @map.id, id: page.friendly_id, import: { file: file } }, session: valid_session
      ...
      context 'with invalid CSV' do
        ...
        it 'shows an error message' do
          post :import_preview, params: { map_id: @map.id, id: page.friendly_id, import: { file: invalid_file } }, session: valid_session
    ...
    describe 'POST #importing' do
      ...
      context 'without data stored in cache' do
        it 'redirects to map_page_path and flashes error message' do
          post :importing, params: { map_id: @map.id, id: page.friendly_id, file: file, import_mapping_id: import_mapping.id }, session: valid_session
          expect(response).to redirect_to(import_map_page_path(@map, page))
      ...
      context 'with data stored in cache' do
        context 'with valid CSV' do
          before do
            importing_rows = [Place.new(title: 'Place 1', lat: 53.95, lon: 9.34, page: page), Place.new(title: 'Place 2', lat: 53.85, lon: 9.27, page: page)]
            allow(ImportContextHelper).to receive(:read_importing_rows).and_return(importing_rows)
          end

          it 'imports the CSV and redirects to map_page_path' do
            post :importing, params: { map_id: @map.id, id: page.friendly_id, file: file, import_mapping_id: import_mapping.id }, session: valid_session

            expect(response).to redirect_to(map_page_path(@map, page))
            expect(flash[:notice]).to match("CSV import to #{page.title} completed successfully!")
          end
        ...
        context 'with invalid CSV' do
          it 'does not import the CSV and shows an error message' do
            post :importing, params: { map_id: @map.id, id: page.friendly_id, file: invalid_file, import_mapping_id: import_mapping.id }, session: valid_session
    ...
    describe 'GET #pack' do
      it 'returns a success response' do
        page = Layer.create! valid_attributes
        get :pack, params: { map_id: @map.id, id: page.friendly_id }, session: valid_session
        expect(response).to have_http_status(200)
      end
    end
    ...
    describe 'GET #build' do
      xit 'returns a success response' do
        page = Layer.create! valid_attributes
        patch :build, params: { map_id: @map.id, id: page.id }, session: valid_session
        expect(response).to have_http_status(200)
      end
    end
    ...
    describe 'GET #show' do
      ...
      it 'returns a sucess with use_background_from_parent_map=true' do
        page = FactoryBot.create(:page, :use_background_from_parent_map, map: @map)
        get :show, params: { map_id: @map.friendly_id, id: page.friendly_id }, session: valid_session
        expect(response).to have_http_status(200)
        expect(assigns(:page)['use_background_from_parent_map']).to be_truthy
        expect(assigns(:page)['basemap_url']).to eq(@map.basemap_url)
      end
    end
    ...
    describe 'GET #show as json' do
      ...
      it 'a page with all places' do
        page = Layer.create! valid_attributes
        page = FactoryBot.create(:page, map_id: @map.id, published: true)
        p1 = FactoryBot.create(:place, :published, page_id: page.id, title: 'Place1')
        p2 = FactoryBot.create(:place, :published, page_id: page.id, title: 'Place2')
        p3 = FactoryBot.create(:place, page_id: page.id, title: 'Place3')
        get :show, params: { id: page.friendly_id, map_id: @map.friendly_id }, session: valid_session, format: 'json'
        json = JSON.parse(response.body)
      ...
      it 'a page with published places and some attached images' do
        page = Layer.create! valid_attributes
        page = FactoryBot.create(:page, map_id: @map.id, published: true)
        p1 = FactoryBot.create(:place, :with_images, page_id: page.id, title: 'Place1')
        p2 = FactoryBot.create(:place, :with_images, page_id: page.id, title: 'Place2')
        p3 = FactoryBot.create(:place, page_id: page.id, title: 'Place3')
        get :show, params: { id: page.friendly_id, map_id: @map.friendly_id }, session: valid_session, format: 'json'
        json = JSON.parse(response.body)
    ...
    describe 'GET #edit' do
      ...
      it 'returns edit_geojson view if ltype is geojson' do
        page = FactoryBot.create(:page, :geojson, map: @map)
        get :edit, params: { map_id: @map.friendly_id, id: page.friendly_id }, session: valid_session
        expect(response).to have_http_status(200)
      ...
      it 'returns a success response with no color set' do
        page = FactoryBot.create(:page, :with_no_color, map: @map)
        get :edit, params: { map_id: @map.friendly_id, id: page.friendly_id }, session: valid_session
        expect(response).to have_http_status(200)
      end

      it 'returns a success response with_wrong_color_format set' do
        page = FactoryBot.create(:page, :with_wrong_color_format, map: @map)
        get :edit, params: { map_id: @map.friendly_id, id: page.friendly_id }, session: valid_session
        expect(response).to have_http_status(200)
        expect(assigns(:page)['color']).to eq('#cc0000')
      end
    ...
    describe 'POST #create with image page', skip: 'Fix CI Testing for Exif values' do
      let(:image_page) do
      ...
      let(:valid_image_page_attributes) do
        FactoryBot.build(:page, :with_ltype_image, map_id: @map.id).attributes
      end
      ...
      context 'with valid params' do
        ...
        it 'creates a new Layer' do
          expect do
            post :create, params: { map_id: @map.friendly_id, page: valid_image_page_attributes }, session: valid_session
          end.to change(Layer, :count).by(1)
      ...
      context 'with invalid params (images are missing)' do
        it 'doest not create a new Layer' do
          ...
          expect(flash[:alert]).to match 'This is an image page, but no images has been provided.'
        end
    ...
    describe '#validate_images_format' do
      ...
      context 'with image files without geocoding' do
        before do
          valid_image_page_attributes.merge!(images_files: falsey_images_files)
        end
